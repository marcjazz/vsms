...
enum Command {
    ...
    /// `backends/crates/sms-api/examples/send_test_message.rs`'s own fixture-seeding
...
/// an equivalent bootstrap-tool case.
async fn record_route_validation_command(command: Command) -> Result<()> {
    let Command::RecordRouteValidation {
        database_url,
        route_id,
        operator,
        performed_by,
        ...
        unreachable!("only ever called with Command::RecordRouteValidation")
    };
