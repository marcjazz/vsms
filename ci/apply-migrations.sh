...
# backends/migrations/postgres/0003_idempotency_table/up.sql instead — same
...
# loop is safe.
for dir in backends/migrations/postgres/*/; do
  name=$(basename "$dir")
  ...
  if command -v psql >/dev/null 2>&1; then
    ...
    docker run --rm --network host -i postgres:16 psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -q -f - < "$dir/up.sql"
  fi
done
